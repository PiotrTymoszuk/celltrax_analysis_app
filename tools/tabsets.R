# Tabsets for the main panel

# Tab 1: general information ------

  general_info <- function() {

    tabPanel('General information',
             textOutput('test'),
             h3('Welcome to celltrax tracing analysis tools!'),
             hr(),
             p("The celltrax analysis application is an interactive implementation of ",
               a(href = 'https://github.com/ingewortel/celltrackR', 'celltrackR'),
               " and ",
               a(href = 'https://github.com/PiotrTymoszuk/celltrax', 'celltrax'),
               " R packages for pre-processing and analysis of microscopy cell tracing data.
               This online Shiny tool provides a platform for analysis of user-provided
               cell tracking data in text form with most commonly used settings.
               Ideally, the input data should be pre-processed to repair hardware
               and software faults, short tracks or drift, e. g. with the ",
               a(href = 'https://im2-ibk.shinyapps.io/celltrax_preprocess/',
                 'celltrax preprocessing tools app.'),
               "The upper size limit of the input data is 1000 tracks or 40000 steps.
               For analysis of multiple samples, rich or untypical track
               sets (i.e. motile macroscopic objects), please resort to the seminal R packages."),
             br(),
             p("For help, please refer to the ",
               a(href = 'Manual.pdf', 'manual.'),
               "You may be also interested in experimenting with our ",
               a(href = 'well_3_tracks.tsv', 'macrophage demo data'),
               " or ",
               a(href = 'b_cells.tsv', 'B cells'),
               ", ",
               a(href = 't_cells.tsv', 'T cells'),
               " and ",
               a(href = 'neutros.tsv', 'neutrophils'),
               " provided with the celltrackR package."),
             br(),
             p("The R code of the application is open and available ",
               a(href = 'https://github.com/PiotrTymoszuk/celltrax_analysis_app', 'here.')),
             p("The app developers put all efforts to develop and maintain qualitative cell tracing
                solutions but carry no responsibility for correctness and error-free functioning of
                the application. This tool may not be used for diagnostic and treatment purposes.
                For scientific use only."),
             hr(),
             em('By using the application you accept',
                a('the terms of use and licensing', href = 'Readme.pdf')),
             br(),
             HTML("<div style =  'text-align: right'>
                                  <img src = '' width = 80%>
                                  <p>Powered by </p>
                                  <a href = 'http://www.daas.tirol'>
                                  <img src = 'logo_large.png' width = 60 alt = 'daas.tirol'>
                                  </a>
                                  <img src = '' width = 30>
                                   <img src = 'shiny_logo.png' width = 60></div>"))

  }

# Tab 2: track statistics -----

  track_stats <- function() {

    tabPanel('Track statistics',
             h3('Basic cell track statistics'),
             br(),
             p("This analysis enables you to exoplore cell displacements and speed
               (step-wise mean and lengths of total displacement/speed vectors),
               tracks straightness and asphericity as estimates of directional
               movement."),
             hr(),
             fluidRow(column(width = 3,
                             align = 'left',
                             h4('Cell displacement and speed')),
                      column(width = 9,
                             align = 'right',
                             dropdownButton(inputId = 'disp_settings',
                                            width = '100%',
                                            circle = FALSE,
                                            status = 'secondary',
                                            size = 'sm',
                                            tooltip = tooltipOptions(title = 'settings',
                                                                     placement = 'left'),
                                            icon = icon(name = 'menu-hamburger',
                                                        lib = 'glyphicon'),
                                            fluidRow(column(width = 3,
                                                            selectInput(inputId = 'disp_aggregate',
                                                                        label = 'statistic type',
                                                                        choices = list('total' = 'total',
                                                                                       'mean per step' = 'mean',
                                                                                       'median per step' = 'median'),
                                                                        selected = 'total',
                                                                        width = '80%'),
                                                            bsTooltip(id = 'disp_aggregate',
                                                                      title = 'total, mean or median')),
                                                     column(width = 3,
                                                            selectInput(inputId = 'disp_geom',
                                                                        label = 'plot type',
                                                                        choices = list('histogram' = 'histogram',
                                                                                       'violin plot' = 'violin',
                                                                                       'box plot' = 'boxplot'),
                                                                        selected = 'violin',
                                                                        width = '80%'),
                                                            bsTooltip(id = 'disp_geom',
                                                                      title = 'violin, histogram or box plot')),
                                                     column(width = 3,
                                                            selectInput(inputId = 'disp_color',
                                                                        label = 'plot color',
                                                                        choices = plot_colors(),
                                                                        selected = 'steelblue',
                                                                        width = '80%'),
                                                            bsTooltip(id = 'disp_color',
                                                                      title = 'data point and shape color')))))),
             br(),
             fluidRow(column(width = 3,
                             plotOutput(outputId = 'disp_plot',
                                        height = '400px',
                                        width = '100%'),
                             bsTooltip(id = 'disp_plot',
                                       title = 'violin or boxplot, horizontal lines code for 25%, 50% and 75% percentiles')),
                      column(width = 1,
                             align = 'left',
                             downloadBttn(outputId = 'download_disp',
                                          label = 'download plot',
                                          style = 'material-circle',
                                          size = 'xs',
                                          color = 'default',
                                          block = TRUE),
                             bsTooltip(id = 'test1',
                                       title = 'download the displacement plot')),
                      column(width = 3,
                             plotOutput(outputId = 'speed_plot',
                                        height = '400px',
                                        width = '100%'),
                             bsTooltip(id = 'speed_plot',
                                       title = 'violin or boxplot, horizontal lines code for 25%, 50% and 75% percentiles')),
                      column(width = 1,
                             align = 'left',
                             downloadBttn(outputId = 'download_speed',
                                          label = 'download plot',
                                          style = 'material-circle',
                                          size = 'xs',
                                          color = 'default',
                                          block = TRUE),
                             bsTooltip(id = 'download_speed',
                                       title = 'download the speed plot'))),
             hr(),
             fluidRow(column(width = 3,
                             align = 'left',
                             h4('Track straightness')),
                      column(width = 9,
                             align = 'right',
                             dropdownButton(width = '100%',
                                            circle = FALSE,
                                            status = 'secondary',
                                            size = 'sm',
                                            tooltip = tooltipOptions(title = 'settings',
                                                                     placement = 'left'),
                                            icon = icon(name = 'menu-hamburger',
                                                        lib = 'glyphicon'),
                                            fluidRow(column(width = 3,
                                                            selectInput(inputId = 'straight_geom',
                                                                        label = 'plot type',
                                                                        choices = list('histogram' = 'histogram',
                                                                                       'violin plot' = 'violin',
                                                                                       'box plot' = 'boxplot'),
                                                                        selected = 'violin',
                                                                        width = '80%'),
                                                            bsTooltip(id = 'straight_geom',
                                                                      title = 'violin. histogram or box plot')),
                                                     column(width = 3,
                                                            selectInput(inputId = 'straight_color',
                                                                        label = 'plot color',
                                                                        choices = plot_colors(),
                                                                        selected = 'steelblue',
                                                                        width = '80%'),
                                                            bsTooltip(id = 'straight_color',
                                                                      title = 'data point and shape color')))))),
             br(),
             fluidRow(column(width = 3,
                             plotOutput(outputId = 'straight_plot',
                                        height = '400px',
                                        width = '100%'),
                             bsTooltip(id = 'straight_plot',
                                       title = 'violin or boxplot, horizontal lines code for 25%, 50% and 75% percentiles')),
                      column(width = 1,
                             align = 'left',
                             downloadBttn(outputId = 'download_straight',
                                          label = 'download plot',
                                          style = 'material-circle',
                                          size = 'xs',
                                          color = 'default',
                                          block = TRUE),
                             bsTooltip(id = 'download_straight',
                                       title = 'download the straightness plot')),
                      column(width = 3,
                             plotOutput(outputId = 'aspher_plot',
                                        height = '400px',
                                        width = '100%'),
                             bsTooltip(id = 'aspher_plot',
                                       title = 'violin or boxplot, horizontal lines code for 25%, 50% and 75% percentiles')),
                      column(width = 1,
                             align = 'left',
                             downloadBttn(outputId = 'download_aspher',
                                          label = 'download plot',
                                          style = 'material-circle',
                                          size = 'xs',
                                          color = 'default',
                                          block = TRUE),
                             bsTooltip(id = 'download_aspher',
                                       title = 'download the asphericity plot')))
             )

  }

# Tab 3: mode of motility -------

  motility <- function() {

    tabPanel('Type of motion',
                h3('Type of motion assessed by Gaussian modeling'),
                br(),
                p("Non-motile objects in your sample may represent dead cells.
                  In turn, non-adherent floating cells may underlie random,
                  Brownian-like motion. By principle, non-motile or passively moving
                  cells may be well modeled by fitting a single Gaussian distribution
                  to their position coordinates. Yet, this simple model fails for
                  actively moving cells. See: ",
                  a('celltraceR manual',
                    href = 'https://cran.rstudio.com/web/packages/celltrackR/vignettes/QC.html'),
                  " for details. The difference in the active and passive motility model fits
                  is described by 'delta BIC' parameter. Low delta BIC values indicate
                  Brownian-like motility, high delta BIC values suggest active movement and
                  can be used to identify passively moving cells in your sample and eliminate them.
                  The 'sigma' parameter estimates the passive motility radius around a central point:
                  approximate cell diameter is usually a good starting value."),
                hr(),
                fluidRow(column(width = 3,
                                align = 'left',
                                h4('delta BIC and track stats')),
                         column(width = 9,
                                align = 'right',
                                dropdownButton(width = '100%',
                                               circle = FALSE,
                                               status = 'secondary',
                                               size = 'sm',
                                               tooltip = tooltipOptions(title = 'settings',
                                                                        placement = 'left'),
                                               icon = icon(name = 'menu-hamburger',
                                                           lib = 'glyphicon'),
                                               fluidRow(column(width = 2,
                                                               numericInput(inputId = 'sigma',
                                                                            label = 'sigma',
                                                                            value = 10,
                                                                            min = 0,
                                                                            width = '100%'),
                                                               bsTooltip(id = 'sigma',
                                                                         title = 'estimate of random movement radius: approx. cell diameter')),
                                                        column(width = 2,
                                                               selectInput(inputId = 'bic_color',
                                                                           label = 'plot color',
                                                                           choices = plot_colors(),
                                                                           selected = 'steelblue',
                                                                           width = '100%'),
                                                               bsTooltip(id = 'bic_color',
                                                                         title = 'data point color')),
                                                        column(width = 2,
                                                               numericInput(inputId = 'bic_coverage',
                                                                            label = '% plotted cells',
                                                                            value = 100,
                                                                            min = 10,
                                                                            max = 100,
                                                                            step = 10,
                                                                            width = '100%'),
                                                               bsTooltip(id = 'bic_coverage',
                                                                         title = '% of tracks plotted, random selection if < 100%')),
                                                        column(width = 2,
                                                               selectInput(inputId = 'bic_x_scale',
                                                                           label = 'X axis',
                                                                           choices = list('identity' = 'identity',
                                                                                          'pseudo log' = 'pseudo_log'),
                                                                           selected = 'identity',
                                                                           width = '100%'),
                                                               bsTooltip(id = 'bic_x_scale',
                                                                         title = 'transformation of the X axis')),
                                                        column(width = 2,
                                                               selectInput(inputId = 'bic_y_scale',
                                                                           label = 'Y axis',
                                                                           choices = list('identity' = 'identity',
                                                                                          'pseudo log' = 'pseudo_log'),
                                                                           selected = 'identity',
                                                                           width = '100%'),
                                                               bsTooltip(id = 'bic_y_scale',
                                                                         title = 'transformation of the Y axis')))))),
                br(),
                fluidRow(column(width = 5,
                                plotOutput('bic_displ',
                                           height = '500px',
                                           width = '100%'),
                                bsTooltip(id = 'bic_displ',
                                          title = 'high delta BIC and total displacement suggest active cell motility')),
                         column(width = 1,
                                align = 'left',
                                downloadBttn(outputId = 'download_bic_disp',
                                             label = 'download plot',
                                             style = 'material-circle',
                                             size = 'xs',
                                             color = 'default',
                                             block = TRUE),
                                bsTooltip(id = 'download_bic_disp',
                                          title = 'download the delta BIC - displacement plot')),
                         column(width = 5,
                                plotOutput('bic_aspher',
                                           height = '500px',
                                           width = '100%'),
                                bsTooltip(id = 'bic_aspher',
                                          title = 'high delta BIC and asphericity suggest active directional cell motility')),
                         column(width = 1,
                                align = 'left',
                                downloadBttn(outputId = 'download_bic_aspher',
                                             label = 'download plot',
                                             style = 'material-circle',
                                             size = 'xs',
                                             color = 'default',
                                             block = TRUE),
                                bsTooltip(id = 'download_bic_disp',
                                          title = 'download the delta BIC - asphericity plot'))))

  }

# Tab 4: autocovariance ------

  autocov_plots <- function() {

    tabPanel('Auto-covariance',
             h3('Movement persistence: autoc-ovariance of displacement vectors'),
             br(),
             p("By investigating of covariance of subsequent displacement vectors of a track,
               persistent, directional motion of a cell may be investigated. This tool
               offers two methods of auto-covariance calculation: by computing dot products
               and by measuring angles between the subsequent displacement vectors of each
               track. Mean statistic per step is presented."),
             hr(),
             fluidRow(column(width = 3,
                             align = 'left',
                             h4('Auto-covariance')),
                      column(width = 9,
                             align = 'right',
                             dropdownButton(width = '100%',
                                            circle = FALSE,
                                            status = 'secondary',
                                            size = 'sm',
                                            tooltip = tooltipOptions(title = 'settings',
                                                                     placement = 'left'),
                                            icon = icon(name = 'menu-hamburger',
                                                        lib = 'glyphicon'),
                                            fluidRow(column(width = 3,
                                                            selectInput(inputId = 'autocov_color',
                                                                        label = 'plot color',
                                                                        choices = plot_colors(),
                                                                        selected = 'steelblue',
                                                                        width = '80%'),
                                                            bsTooltip(id = 'autocov_color',
                                                                      title = 'data point and line color')),
                                                     column(width = 3,
                                                            selectInput(inputId = 'autocov_y_scale',
                                                                        label = 'Y axis',
                                                                        choices = list('identity' = 'identity',
                                                                                       'pseudo log' = 'pseudo_log'),
                                                                        selected = 'identity',
                                                                        width = '80%'),
                                                            bsTooltip(id = 'autocov_y_scale',
                                                                      title = 'transformation of the Y axis')))))),
             br(),
             br(),
             fluidRow(column(width = 5,
                             plotOutput(outputId = 'autocov_dot',
                                        height = '500px',
                                        width = '100%'),
                             bsTooltip(id = 'autocov_dot',
                                       title = 'dot product > 0 suggest persistent motion in one direction')),
                      column(width = 1,
                             align = 'left',
                             downloadBttn(outputId = 'download_auto_dot',
                                          label = 'download plot',
                                          style = 'material-circle',
                                          size = 'xs',
                                          color = 'default',
                                          block = TRUE),
                             bsTooltip(id = 'download_auto_dot',
                                       title = 'download the dot product plot')),
                      column(width = 5,
                             plotOutput(outputId = 'autocov_angle',
                                        height = '500px',
                                        width = '100%'),
                             bsTooltip(id = 'autocov_angle',
                                       title = 'angle < 90 degree or < 1.6 radians suggests persistent motion in one direction')),
                      column(width = 1,
                             align = 'left',
                             downloadBttn(outputId = 'download_auto_angle',
                                          label = 'download plot',
                                          style = 'material-circle',
                                          size = 'xs',
                                          color = 'default',
                                          block = TRUE),
                             bsTooltip(id = 'download_auto_angle',
                                       title = 'download the angle plot'))))

  }

# Tab 5: pair analysis ------

  pair_analysis <- function() {

    tabPanel('Pair analysis',
             h3('Analysis of cell pairs'),
             br(),
             p("By analysing distances and angles for all cell pairs in the sample,
               a tendency towards concomitant directional motility can be discerned.
               If the cells of your sample move randomly, the angles between cell pair's
               displacement vectors are expected to be evenly distrubuted between 0
               and 180 degrees and mean of approximately 90 degrees. If a specific
               directional motility takes place, such angles are expected to be low,
               between 0 and 90 degrees, independently of the cell - cell distance.
               This tools calculates distances and angles between displacement
               vectors for each cell pair or single steps
               in the track. The expected angle value as a function
               of the cell - cell distance is estimated by a GAM trand
               (generalized additive model) with 95% confidence interval."),
             hr(),
             fluidRow(column(width = 3,
                             align = 'left',
                             h4('Cell pair distances and angles')),
                      column(width = 9,
                             align = 'right',
                             dropdownButton(width = '100%',
                                            circle = FALSE,
                                            status = 'secondary',
                                            size = 'sm',
                                            tooltip = tooltipOptions(title = 'settings',
                                                                     placement = 'left'),
                                            icon = icon(name = 'menu-hamburger',
                                                        lib = 'glyphicon'),
                                            fluidRow(column(width = 3,
                                                            selectInput(inputId = 'pair_color',
                                                                        label = 'point color',
                                                                        choices = plot_colors(),
                                                                        selected = 'steelblue',
                                                                        width = '80%'),
                                                            bsTooltip(id = 'pair_color',
                                                                      title = 'data point color')),
                                                     column(width = 3,
                                                            selectInput(inputId = 'gam_color',
                                                                        label = 'trend color',
                                                                        choices = plot_colors(),
                                                                        selected = 'orangered3',
                                                                        width = '80%'),
                                                            bsTooltip(id = 'gam_color',
                                                                      title = 'color of the trend line')),
                                                     column(width = 3,
                                                            numericInput(inputId = 'pair_coverage',
                                                                         label = '% analyzed tracks',
                                                                         value = 100,
                                                                         min = 10,
                                                                         max = 100,
                                                                         width = '80%'),
                                                            bsTooltip(id = 'pair_coverage',
                                                                      title = '% of tracks plotted, random selection if < 100%')))))),
             br(),
             fluidRow(column(width = 5,
                             plotOutput(outputId = 'pair_cells',
                                        height = '500px',
                                        width = '100%'),
                             bsTooltip(id = 'pair_cells',
                                       title = 'trend of angle < 90 degrees indicates directional motility')),
                      column(width = 1,
                             align = 'left',
                             downloadBttn(outputId = 'download_cells',
                                          label = 'download plot',
                                          style = 'material-circle',
                                          size = 'xs',
                                          color = 'default',
                                          block = TRUE),
                             bsTooltip(id = 'download_cells',
                                       title = 'download the cell analysis plot')),
                      column(width = 5,
                             plotOutput(outputId = 'pair_steps',
                                        height = '500px',
                                        width = '100%'),
                             bsTooltip(id = 'pair_steps',
                                       title = 'trend of angle < 90 degrees indicates directional motility')),
                      column(width = 1,
                             align = 'left',
                             downloadBttn(outputId = 'download_steps',
                                          label = 'download plot',
                                          style = 'material-circle',
                                          size = 'xs',
                                          color = 'default',
                                          block = TRUE),
                             bsTooltip(id = 'download_steps',
                                       title = 'download the step analysis plot'))))

  }

# Tab 6: directional movement --------

  dir_analysis <- function() {

    tabPanel('Directional movement',
             h3('Analysis of directional movement'),
             br(),
             p("By visual analysis of cell tracks and total displacement vectors,
               a tendency towards directional movement can be observed."),
             hr(),
             fluidRow(column(width = 3,
                             align = 'left',
                             h4('Tracks and total displacement')),
                      column(width = 9,
                             align = 'right',
                             dropdownButton(width = '100%',
                                            circle = FALSE,
                                            status = 'secondary',
                                            size = 'sm',
                                            tooltip = tooltipOptions(title = 'settings',
                                                                     placement = 'left'),
                                            icon = icon(name = 'menu-hamburger',
                                                        lib = 'glyphicon'),
                                            fluidRow(column(width = 3,
                                                            radioButtons(inputId = 'monochrome',
                                                                         label = 'monochrome',
                                                                         choices = list('no' = 'no',
                                                                                        'yes' = 'yes'),
                                                                         selected = 'no',
                                                                         inline = TRUE,
                                                                         width = '80%'),
                                                            bsTooltip(id = 'monochrome',
                                                                      title = 'should each track be plotted with a separate color?')),
                                                     column(width = 3,
                                                            selectInput(inputId = 'track_color',
                                                                        label = 'track/vector color',
                                                                        choices = plot_colors(),
                                                                        selected = 'steelblue',
                                                                        width = '80%'),
                                                            bsTooltip(id = 'track_color',
                                                                      title = 'color of tracks and vectors, valid only if monochrome set to yes')),
                                                     column(width = 3,
                                                            selectInput(inputId = 'velo_color',
                                                                        label = 'mean vector color',
                                                                        choices = plot_colors(),
                                                                        selected = 'orangered3',
                                                                        width = '80%'),
                                                            bsTooltip(id = 'velo_color',
                                                                      title = "color of the mean displacement vector")),
                                                     column(width = 3,
                                                            numericInput(inputId = 'track_coverage',
                                                                         label = '% plotted tracks',
                                                                         value = 100,
                                                                         min = 10,
                                                                         max = 100,
                                                                         width = '80%'),
                                                            bsTooltip(id = 'track_coverage',
                                                                      title = '% of tracks plotted, random selection if < 100%')))))),
             br(),
             fluidRow(column(width = 7,
                             plotOutput(outputId = 'tracks_raw',
                                        height = '600px',
                                        width = '100%'),
                             bsTooltip(id = 'tracks_raw',
                                       title = 'open circle indicates the final point of the track')),
                      column(width = 5,
                             dropdownButton(width = '100%',
                                            icon = icon(name = 'cog',
                                                        lib = 'glyphicon'),
                                            status = 'secondary',
                                            size = 'sm',
                                            tooltip = tooltipOptions(title = 'axes and download'),
                                            fluidRow(column(width = 6,
                                                            br(),
                                                            br(),
                                                            numericInputIcon(inputId = 'x_min_tracks_raw',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-horizontal',
                                                                                              lib = 'glyphicon'),
                                                                                         'min')),
                                                            bsTooltip(id = 'x_min_tracks_raw',
                                                                      title = 'X axis minimum'),
                                                            numericInputIcon(inputId = 'y_min_tracks_raw',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-vertical',
                                                                                              lib = 'glyphicon'),
                                                                                         'min')),
                                                            bsTooltip(id = 'y_min_tracks_raw',
                                                                      title = 'Y axis minimum'),
                                                            actionBttn(inputId = 'tracks_raw_refresh',
                                                                       label = 'reset',
                                                                       style = 'material-circle',
                                                                       color = 'default',
                                                                       size = 'xs',
                                                                       icon = icon(name = 'refresh',
                                                                                   lib = 'glyphicon')),
                                                            bsTooltip(id = 'tracks_raw_refresh',
                                                                      title = 'reset axis ranges')),
                                                     column(width = 6,
                                                            br(),
                                                            br(),
                                                            numericInputIcon(inputId = 'x_max_tracks_raw',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-horizontal',
                                                                                              lib = 'glyphicon'),
                                                                                         'max')),
                                                            bsTooltip(id = 'x_max_tracks_raw',
                                                                      title = 'X axis maximum'),
                                                            numericInputIcon(inputId = 'y_max_tracks_raw',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-vertical',
                                                                                              lib = 'glyphicon'),
                                                                                         'max')),
                                                            bsTooltip(id = 'y_max_tracks_raw',
                                                                      title = 'Y axis maximum'),
                                                            downloadBttn(outputId = 'download_tracks_raw',
                                                                         label = 'download plot',
                                                                         style = 'material-circle',
                                                                         size = 'xs',
                                                                         color = 'default',
                                                                         block = TRUE),
                                                            bsTooltip(id = 'download_tracks_raw',
                                                                      title = 'download the track plot')))))),
             br(),
             br(),
             fluidRow(column(width = 7,
                             plotOutput(outputId = 'tracks_norm',
                                        height = '600px',
                                        width = '100%'),
                             bsTooltip(id = 'tracks_norm',
                                       title = 'open circle indicates the final point of the track')),
                      column(width = 5,
                             dropdownButton(width = '100%',
                                            icon = icon(name = 'cog',
                                                        lib = 'glyphicon'),
                                            status = 'secondary',
                                            size = 'sm',
                                            tooltip = tooltipOptions(title = 'axes and download'),
                                            fluidRow(column(width = 6,
                                                            br(),
                                                            br(),
                                                            numericInputIcon(inputId = 'x_min_tracks_norm',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-horizontal',
                                                                                              lib = 'glyphicon'),
                                                                                         'min')),
                                                            bsTooltip(id = 'x_min_tracks_norm',
                                                                      title = 'X axis minimum'),
                                                            numericInputIcon(inputId = 'y_min_tracks_norm',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-vertical',
                                                                                              lib = 'glyphicon'),
                                                                                         'min')),
                                                            bsTooltip(id = 'y_min_tracks_norm',
                                                                      title = 'Y axis minimum'),
                                                            actionBttn(inputId = 'tracks_norm_refresh',
                                                                       label = 'reset',
                                                                       style = 'material-circle',
                                                                       color = 'default',
                                                                       size = 'xs',
                                                                       icon = icon(name = 'refresh',
                                                                                   lib = 'glyphicon')),
                                                            bsTooltip(id = 'tracks_norm_refresh',
                                                                      title = 'reset axis ranges')),
                                                     column(width = 6,
                                                            br(),
                                                            br(),
                                                            numericInputIcon(inputId = 'x_max_tracks_norm',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-horizontal',
                                                                                              lib = 'glyphicon'),
                                                                                         'max')),
                                                            bsTooltip(id = 'x_max_tracks_norm',
                                                                      title = 'X axis maximum'),
                                                            numericInputIcon(inputId = 'y_max_tracks_norm',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-vertical',
                                                                                              lib = 'glyphicon'),
                                                                                         'max')),
                                                            bsTooltip(id = 'y_max_tracks_norm',
                                                                      title = 'Y axis maximum'),
                                                            downloadBttn(outputId = 'download_tracks_norm',
                                                                         label = 'download plot',
                                                                         style = 'material-circle',
                                                                         size = 'xs',
                                                                         color = 'default',
                                                                         block = TRUE),
                                                            bsTooltip(id = 'download_tracks_norm',
                                                                      title = 'download the track plot')))))),
             br(),
             br(),
             fluidRow(column(width = 7,
                             plotOutput(outputId = 'vectors_raw',
                                        height = '600px',
                                        width = '100%'),
                             bsTooltip(id = 'vectors_raw',
                                       title = 'vectors connect the start and final point of tracks')),
                      column(width = 5,
                             dropdownButton(width = '100%',
                                            icon = icon(name = 'cog',
                                                        lib = 'glyphicon'),
                                            status = 'secondary',
                                            size = 'sm',
                                            tooltip = tooltipOptions(title = 'axes and download'),
                                            fluidRow(column(width = 6,
                                                            br(),
                                                            br(),
                                                            numericInputIcon(inputId = 'x_min_vectors_raw',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-horizontal',
                                                                                              lib = 'glyphicon'),
                                                                                         'min')),
                                                            bsTooltip(id = 'x_min_vectors_raw',
                                                                      title = 'X axis minimum'),
                                                            numericInputIcon(inputId = 'y_min_vectors_raw',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-vertical',
                                                                                              lib = 'glyphicon'),
                                                                                         'min')),
                                                            bsTooltip(id = 'y_min_vectors_raw',
                                                                      title = 'Y axis mimimum'),
                                                            actionBttn(inputId = 'vectors_raw_refresh',
                                                                       label = 'reset',
                                                                       style = 'material-circle',
                                                                       color = 'default',
                                                                       size = 'xs',
                                                                       icon = icon(name = 'refresh',
                                                                                   lib = 'glyphicon')),
                                                            bsTooltip(id = 'vectors_raw_refresh',
                                                                      title = 'reset axis ranges')),
                                                     column(width = 6,
                                                            br(),
                                                            br(),
                                                            numericInputIcon(inputId = 'x_max_vectors_raw',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-horizontal',
                                                                                              lib = 'glyphicon'),
                                                                                         'max')),
                                                            bsTooltip(id = 'x_max_vectors_raw',
                                                                      title = 'X axis maximum'),
                                                            numericInputIcon(inputId = 'y_max_vectors_raw',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-vertical',
                                                                                              lib = 'glyphicon'),
                                                                                         'max')),
                                                            bsTooltip(id = 'y_max_vectors_raw',
                                                                      title = 'Y axis maximum'),
                                                            downloadBttn(outputId = 'download_vectors_raw',
                                                                         label = 'download plot',
                                                                         style = 'material-circle',
                                                                         size = 'xs',
                                                                         color = 'default',
                                                                         block = TRUE),
                                                            bsTooltip(id = 'download_vectors_raw',
                                                                      title = 'download the vector plot')))))),
             br(),
             fluidRow(column(width = 7,
                             plotOutput(outputId = 'vectors_norm',
                                        height = '600px',
                                        width = '100%'),
                             bsTooltip(id = 'vectors_norm',
                                       title = 'vectors connect the start and final point of tracks')),
                      column(width = 5,
                             dropdownButton(width = '100%',
                                            icon = icon(name = 'cog',
                                                        lib = 'glyphicon'),
                                            status = 'secondary',
                                            size = 'sm',
                                            tooltip = tooltipOptions(title = 'axes and download'),
                                            fluidRow(column(width = 6,
                                                            br(),
                                                            br(),
                                                            numericInputIcon(inputId = 'x_min_vectors_norm',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-horizontal',
                                                                                              lib = 'glyphicon'),
                                                                                         'min')),
                                                            bsTooltip(id = 'x_min_vectors_norm',
                                                                      title = 'X axis minimum'),
                                                            numericInputIcon(inputId = 'y_min_vectors_norm',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-vertical',
                                                                                              lib = 'glyphicon'),
                                                                                         'min')),
                                                            bsTooltip(id = 'y_min_vectors_norm',
                                                                      title = 'Y axis minimum'),
                                                            actionBttn(inputId = 'vectors_norm_refresh',
                                                                       label = 'reset',
                                                                       style = 'material-circle',
                                                                       color = 'default',
                                                                       size = 'xs',
                                                                       icon = icon(name = 'refresh',
                                                                                   lib = 'glyphicon')),
                                                            bsTooltip(id = 'vectors_norm_refresh',
                                                                      title = 'reset axis ranges')),
                                                     column(width = 6,
                                                            br(),
                                                            br(),
                                                            numericInputIcon(inputId = 'x_max_vectors_norm',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-horizontal',
                                                                                              lib = 'glyphicon'),
                                                                                         'max')),
                                                            bsTooltip(id = 'x_max_vectors_norm',
                                                                      title = 'X axis maximum'),
                                                            numericInputIcon(inputId = 'y_max_vectors_norm',
                                                                             label = NULL,
                                                                             value = 0,
                                                                             icon = list(icon(name = 'resize-vertical',
                                                                                              lib = 'glyphicon'),
                                                                                         'max')),
                                                            bsTooltip(id = 'y_max_vectors_norm',
                                                                      title = 'Y axis maximum'),
                                                            downloadBttn(outputId = 'download_vectors_norm',
                                                                         label = 'download plot',
                                                                         style = 'material-circle',
                                                                         size = 'xs',
                                                                         color = 'default',
                                                                         block = TRUE),
                                                            bsTooltip(id = 'download_vectors_norm',
                                                                      title = 'download the vector plot')))))))


  }

# Tab 7: Hotelling's test and plots ------

  hotellings <- function() {

    tabPanel("Hotelling's test",
             h3("Assess significance of directional movement with Hotelling's test"),
             br(),
             p("Formally, directional motility tendency could be assessed by a bi-variate
               (or, generally, multivariate) variant of the T test, the Hotelling's test.
               In the Hotelling's test, the step spacing parameter indicates
               how many positions are to be left out between the steps that are
               considered for the test. It allows you to account for
               stimulus-independent persistent motion."),
             hr(),
             fluidRow(column(width = 3,
                             align = 'left',
                             h4('Hotellings plot')),
                      column(width = 9,
                             align = 'right',
                             dropdownButton(width = '100%',
                                            circle = FALSE,
                                            status = 'secondary',
                                            size = 'sm',
                                            tooltip = tooltipOptions(title = 'settings',
                                                                     placement = 'left'),
                                            icon = icon(name = 'menu-hamburger',
                                                        lib = 'glyphicon'),
                                            fluidRow(column(width = 3,
                                                            numericInput(inputId = 'step_spacing',
                                                                         label = 'step spacing',
                                                                         value = 2,
                                                                         min = 0,
                                                                         max = 20,
                                                                         width = '80%'),
                                                            bsTooltip(id = 'step_spacing',
                                                                      title = 'positions left out between the steps included in the test')),

                                                     column(width = 3,
                                                            selectInput(inputId = 'hotellings_color',
                                                                        label = 'point color',
                                                                        choices = plot_colors(),
                                                                        selected = 'steelblue',
                                                                        width = '80%'),
                                                            bsTooltip(id = 'hotellings_color',
                                                                      title = 'data point color')),
                                                     column(width = 3,
                                                            numericInput(inputId = 'hotellings_coverage',
                                                                         label = '% plotted tracks',
                                                                         value = 100,
                                                                         min = 10,
                                                                         max = 100,
                                                                         width = '80%'),
                                                            bsTooltip(id = 'hotellings_coverage',
                                                                      title = '% of tracks plotted, random selection if < 100%')))))),
             br(),
             fluidRow(column(width = 3),
                      column(width = 6,
                             plotOutput(outputId = 'hotellings_plot',
                                        height = '500px',
                                        width = '100%'),
                             bsTooltip(id = 'hotellings_plot',
                                       title = 'points represent displacements, red point with ellipse denotes expected displacement with 95% confidence region')),
                      column(width = 1,
                             align = 'left',
                             downloadBttn(outputId = 'download_hotellings',
                                          label = 'download plot',
                                          style = 'material-circle',
                                          size = 'xs',
                                          color = 'default',
                                          block = TRUE),
                             bsTooltip(id = 'download_hotellings',
                                       title = "download the Hotelling's plot")),
                      column(width = 2)))


  }

# Tab 8: heterogenity ------

  hetero_analysis <- function() {

    tabPanel('Heterogeneity',
             h3('Heterogenous mode of motility in the sample'),
             br(),
             p("Dimensionality reduction of multiple track measures with
               PCA (principal component analysis),
               UMAP (uniform manifold approximation and Projection, see: DOI 10.1038/nbt.4314
               and ArXiv ID 1802.03426) or MDS (multi-dimensional scaling) may help
               to identify populations of cells in the sample differing in modes of
               their motility. This tool enables two - five dimensional PCA, UMAP or MDS analysis
               of the following track statistics: step number, tracking duration,
               delta BIC (passive versus active motility), total displacement, mean speed,
               overall angle between total displacement vectors, dot product of total
               displacement vectors, track asphericity, X and Y unit vector displacements.
               Prior to analysis, the data set is subjected to normalization
               with median or mean centering."),
             hr(),
             fluidRow(column(width = 3,
                             align = 'left',
                             h4('Dimension variance and scores')),
                      column(width = 9,
                             align = 'right',
                             dropdownButton(inputId = 'red_settings',
                                            width = '100%',
                                            circle = FALSE,
                                            status = 'secondary',
                                            size = 'sm',
                                            tooltip = tooltipOptions(title = 'settings',
                                                                     placement = 'left'),
                                            icon = icon(name = 'menu-hamburger',
                                                        lib = 'glyphicon'),
                                            fluidRow(column(width = 2,
                                                            selectInput(inputId = 'center_method',
                                                                        label = 'data centering',
                                                                        choices = list('mean' = 'mean',
                                                                                       'median' = 'median'),
                                                                        selected = 'mean',
                                                                        width = '100%'),
                                                            bsTooltip(id = 'center_method',
                                                                      title = 'data normalization method, mean or median centering')),
                                                     column(width = 2,
                                                            numericInput(inputId = 'dim_number',
                                                                         label = 'dimensions',
                                                                         value = 2,
                                                                         min = 2,
                                                                         max = 5,
                                                                         step = 1,
                                                                         width = '100%'),
                                                            bsTooltip(id = 'dim_number',
                                                                      title = 'final number of dimensions')),
                                                     column(width = 2,
                                                            selectInput(inputId = 'red_fun',
                                                                        label = 'algorithm',
                                                                        choices = list('PCA' = 'pca',
                                                                                       'UMAP' = 'umap',
                                                                                       'MDS' = 'mds'),
                                                                        selected = 'pca',
                                                                        width = '100%'),
                                                            bsTooltip(id = 'red_fun',
                                                                      title = 'dimensionality reduction method')),
                                                     column(width = 2,
                                                            selectInput(inputId = 'dist_method',
                                                                        label = 'distance metric',
                                                                        choices = list('Euclidean' = 'euclidean',
                                                                                       'Manhattan' = 'manhattan',
                                                                                       'Cosine' = 'cosine'),
                                                                        selected = 'euclidean',
                                                                        width = '100%'),
                                                            bsTooltip(id = 'dist_method',
                                                                      title = 'distance meric, valid only for UMAP and MDS')),
                                                     column(width = 2,
                                                            selectInput(inputId = 'red_color',
                                                                        label = 'point color',
                                                                        choices = plot_colors(),
                                                                        selected = 'steelblue',
                                                                        width = '100%'),
                                                            bsTooltip(id = 'red_color',
                                                                      title = 'data point color')))))),
             br(),
             fluidRow(column(width = 5,
                             plotOutput(outputId = 'scree_plot',
                                        height = '500px',
                                        width = '100%'),
                             bsTooltip(id = 'scree_plot',
                                       title = '% of total variance associated with the dimensions')),
                      column(width = 1,
                             align = 'left',
                             downloadBttn(outputId = 'download_scree',
                                          label = 'download plot',
                                          style = 'material-circle',
                                          size = 'xs',
                                          color = 'default',
                                          block = TRUE),
                             bsTooltip(id = 'download_scree',
                                       title = 'doanload the variance plot')),
                      column(width = 5,
                             plotOutput(outputId = 'red_plot',
                                        height = '500px',
                                        width = '100%'),
                             bsTooltip(id = 'red_plot',
                                       title = 'dimensionality reduction scores or coordinates for each track')),
                      column(width = 1,
                             align = 'left',
                             downloadBttn(outputId = 'download_red',
                                          label = 'download plot',
                                          style = 'material-circle',
                                          size = 'xs',
                                          color = 'default',
                                          block = TRUE),
                             bsTooltip(id = 'download_red',
                                       title = 'download the score plot'))))

  }

# Tab 9: results download ------

  get_data <- function() {

    tabPanel('Fetch data',
             h3('Download track statistics and result plots'),
             br(),
             p("This tool enables you to save the analysis results in tabular
             form including tracks data table, a table with track statistics,
             auto-covariance, Hotelling's test and dimensionality reduction
             results. Finally, you may save the entire analysis bundle
             (tracks/trax object, plots and statistic tables) as a .RDa
             R data storage file and continue with analysis and plot adjustment
             in R."),
             hr(),
             h4('Download result tables and R data bundle'),
             br(),
             fluidRow(column(width = 3,
                             p('Tracks in a tab-delimited text file'),
                             downloadButton(outputId = 'download_trax',
                                            label = 'download tracks'),
                             bsTooltip(id = 'download_trax',
                                       title = 'file with track id, time and coordinates')),
                      column(width = 3,
                             p("Summary of track statistics"),
                             downloadButton(outputId = 'download_stats',
                                            label = 'download stats'),
                             bsTooltip(id = 'download_stats',
                                       title = "track statistics, autocovariance and Hotelling test results")),
                      column(width = 3,
                             p('Dimensionality reduction results'),
                             downloadButton(outputId = 'download_red_tbl',
                                            label = 'download results'),
                             bsTooltip(id = 'download_red_tbl',
                                       title = 'track scores or coordinates after dimensionality reduction')),
                      column(width = 3,
                             p('R data bundle'),
                             downloadButton(outputId = 'download_bundle',
                                            label = 'download R data'),
                             bsTooltip(id = 'download_bundle',
                                       title = 'RDa file with tracks, statistics and editable plots'))))

  }

# END -----
